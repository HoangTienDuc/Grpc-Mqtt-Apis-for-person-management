...
def preprocess(img, input_shape, letter_box=True):
    """
    Preprocess an input image for a deep learning model.

    This function resizes and normalizes the image based on the specified input shape.
    If letter_box is True, it applies letter-boxing to maintain the aspect ratio of the original image.

    Parameters:
    img (numpy.ndarray): Input image in BGR format.
    input_shape (tuple): Desired input shape for the deep learning model (height, width).
    letter_box (bool): Whether to apply letter-boxing. Default is True.

    Returns:
    numpy.ndarray: Preprocessed image in RGB format, normalized to the range [0, 1], and transposed to (channels, height, width).
    """
    if letter_box:
...
def postprocess(num_dets, det_boxes, det_scores, det_classes, img_w, img_h, input_shape, letter_box=True):
    """
    Postprocess the detection results from a deep learning model.

    This function takes the raw detection outputs and performs necessary transformations to obtain the final bounding boxes,
    scores, and classes. It also applies letter-boxing if specified.

    Parameters:
    num_dets (numpy.ndarray): Array containing the number of detections for each class.
    det_boxes (numpy.ndarray): Array containing the bounding box coordinates of detections.
    det_scores (numpy.ndarray): Array containing the confidence scores of detections.
    det_classes (numpy.ndarray): Array containing the class labels of detections.
    img_w (int): Width of the original input image.
    img_h (int): Height of the original input image.
    input_shape (tuple): Desired input shape for the deep learning model (height, width).
    letter_box (bool): Whether to apply letter-boxing. Default is True.

    Returns:
    list: List of BoundingBox objects representing the detected objects.
    """
    ...
    for box, score, label in zip(boxes, scores, classes):
        if label >= len(list(SafetyConstructionLabels)):
            continue
